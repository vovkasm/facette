...
import (
	...
	influxdb "github.com/influxdb/influxdb-legacy/client"
)

// InfluxDBLegacyConnector represents the main structure of the InfluxDB connector.
type InfluxDBLegacyConnector struct {
	name     string
	host     string
	useTLS   bool
	username string
...
func init() {
	Connectors["influxdb-legacy"] = func(name string, settings map[string]interface{}) (Connector, error) {
		var (
			pattern string
			err     error
		)

		connector := &InfluxDBLegacyConnector{
			name:     name,
			host:     "localhost:8086",
			username: "root",
			password: "root",
...
func (connector *InfluxDBLegacyConnector) GetName() string {
...
func (connector *InfluxDBLegacyConnector) GetPlots(query *plot.Query) ([]*plot.Series, error) {
	...
	logger.Log(logger.LevelDebug, "connector", "influxdb-legacy[%s]: executing: %s", connector.name,
		influxdbQuery)

	q, err := connector.client.Query(influxdbQuery, "s")
	if err != nil {
		return nil, fmt.Errorf("influxdb-legacy[%s]: unable to perform query: %s", connector.name, err)
	}
...
func (connector *InfluxDBLegacyConnector) Refresh(originName string, outputChan chan<- *catalog.Record) error {
	seriesList, err := connector.client.Query("select * from /.*/ limit 1")
	if err != nil {
		return fmt.Errorf("influxdb-legacy[%s]: unable to fetch series list: %s", connector.name, err)
	}
	...
	for _, series := range seriesList {
		...
		if len(seriesPoints) == 0 {
			logger.Log(logger.LevelInfo,
				"connector",
				"influxdb-legacy[%s]: series `%s' does not return sample data, ignoring",
				connector.name,
				seriesName,
			)
			continue
		...
		for columnIndex, columnName := range series.GetColumns() {
			...
			if err != nil {
				logger.Log(logger.LevelInfo,
					"connector",
					"influxdb-legacy[%s]: series `%s' does not match pattern, ignoring",
					connector.name,
					seriesName,
				)
				continue
